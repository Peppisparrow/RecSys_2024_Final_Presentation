...
class EASE_R_Recommender(BaseItemSimilarityMatrixRecommender):
    ...
    def fit(self, topK=None, l2_norm = 1e3, normalize_matrix = False, evaluator=None, iterations=200):
        ...
        for i in range(iterations):
            if topK is not None:
                BK = similarityMatrixTopK(B, k = topK-i, use_absolute_values = True, verbose = False)
            else:
                BK = B
    ...
    def tuningK(self, topK, evaluator, iterations = 200, step = 50):
        ...
        for i in range(iterations):
            if topK is not None:
                BK = similarityMatrixTopK(B, k = topK-i*step, use_absolute_values = True, verbose = False)
            else:
                BK = B
